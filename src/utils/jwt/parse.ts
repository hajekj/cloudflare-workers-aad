...
import { DecodedJwt, JwtParseResult } from "@cfworker/jwt/dist/types";
import { verifyJwtSignature } from "@cfworker/jwt/dist/verify";
...
export async function parseJwt(
    ...
    try {
        ...
        return {
            valid: false,
            reason: `Error retrieving key to verify JWT signature. ${JSON.stringify(err)}`
        };
    }
